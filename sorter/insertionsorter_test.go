...
func TestInsertionSort(t *testing.T) {
	r1 := data.Row{"name": data.NewStringValue("Paul"), "age": data.NewIntegerValue(25)}
	...
	r := []data.Row{r1, r2, r3}
	sr := SortableRows(r)
	sorted := sortByInsertion(sr, "age")
	if sorted[0].GetString("name") != "John" {
		t.Error("Expected John, got ", sorted[0].GetString("name"))
	}
	if sorted[1].GetString("name") != "Paul" {
		t.Error("Expected Paul, got ", sorted[1].GetString("name"))
	}
